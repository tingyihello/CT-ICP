...
namespace ct_icp {
    ...
    struct Point3D {
        ...
        Eigen::Vector3d pt; // Corrected point taking into account the motion of the sensor during frame acquisition  根据传感器的运动，修正后的点的坐标
        double alpha_timestamp = 0.0; // Relative timestamp in the frame in [0.0, 1.0]  相对时间
        double timestamp = 0.0; // The absolute timestamp (if applicable)  绝对时间
    ...
    // 判断一个Voxel中包含的点数，以及判断此Voxel是否满或者空
    struct VoxelBlock {
