...
namespace ct_icp {
    ...
    struct OdometryOptions {
        ...
        double init_voxel_size = 0.2; //初始voxel大小

        double init_sample_voxel_size = 1.0; 

        int init_num_frames = 20; // The number of frames defining the initialization of the map

        double voxel_size = 0.5;  // voxel_size和sample_voxel_size的作用？

        double sample_voxel_size = 1.5;

        double max_distance = 100.0; // The threshold on the voxel size to remove points from the map 最大的voxel尺寸？
        ...
        // Whether to assess the quality of the registration,  评估配准的质量
        // And try a new registration with more conservative parameters in case of failure
        int robust_minimal_level = 0;  // 这个参数是干啥的
        bool robust_registration = false;
        double robust_full_voxel_threshold = 0.7;  //判断一个voxel是 满/空 的阈值
        ...
        double robust_relative_trans_threshold = 1.0;  //这个参数的作用？？？
