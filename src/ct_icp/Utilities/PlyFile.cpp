...
void PlyFile::readHeader()
{
	...
	string strTmp = "";
	list<plyTypes> typePptTmp;   // 记录数据的类型
	list<int>      sizePptTmp;   // 记录数据占内存的大小
	...
	while (!streamHeader.eof())
	{
		...
		{
			streamHeader >> strTmp;
			if (strTmp.compare("vertex") == 0) streamHeader >> _numPoints;  //从ply文件的文件头中读取 点的个数
			if (strTmp.compare("camera") == 0) break;
		}
	...
	for (int i(0); i < _propertyNum; i++)
	{
		...
		_pointSize += _propertySize[i];           // 总的空间大小
	}
}
...
void PlyFile::readFile(char*& points, int& pointSize, int& numPoints)
{
	switch (_format)
	{
	case binary_little_endian:   //二进制小字节序
	{
		// ----- Allocate memory ------------------------------------------------
		if (points != 0)
		{
		...
		// ----- Read raw data --------------------------------------------------
		// 为什么要一个READ-SIZE大小读取
		unsigned long long int n = bufferSize / (unsigned long long int)READ_SIZE;  //READ_SIZE=16000
...
{
	// ----- Set properties -------------------------------------------------
	// -----属性指的应该是每个点包含的数据，例如x y z intensity等，并记录每个数据的类型，计算每个点占多少字节
	...
	_propertyType = new plyTypes[_propertyNum];   // 记录的是数据类型
	_propertySize = new int[_propertyNum];
	_propertyName = new string[_propertyNum];
	...
	// ----- Write points ---------------------------------------------------
	unsigned long long int bufferSize = (unsigned long long int)_pointSize*(unsigned long long int)_numPoints; //计算数据部分所需要的文件大小
