...
namespace ct_icp {
    ...
    struct SLAMOptions {
        ...
        bool suspend_on_failure = false; // Whether to suspend the execution once an error is detected 中止程序
...
SLAMOptions read_config(const std::string &config_path) {
    ...
    try {
        ...
        auto &dataset_options = options.dataset_options;  // 这里的“&”是什么意思？

        // 读取配置文件中的 有关dataset的参数
        if (dataset_node["dataset"]) {
            ...
            CHECK(dataset == "KITTI_raw" || dataset == "KITTI_CARLA" || dataset == "KITTI" ||
                  dataset == "KITTI-360" || dataset == "NCLT" || dataset == "PLY_DIRECTORY" || dataset == "OUR_DATA");
            ...
            if (dataset == "OUR_DATA")
                dataset_options.dataset = OUR_DATA;
        }
...
SLAMOptions read_arguments(int argc, char **argv) {
    ...
    try {
        ...
        // Parse the arguments of the command line  读取命令行，判断命令行的参数是否规范
        cmd.parse(argc, argv);
...
int main(int argc, char **argv) {
    ...
    // ***   step3: 处理指定的sequence情况
    auto sequences = ct_icp::get_sequences(options.dataset_options);  //包括每个sequence的id,name,size
    ...
    if (!options.all_sequences) {
        ...
        for (int idx(0); idx < sequences.size(); ++idx) {
            ...
            if (sequence.sequence_name == options.sequence) {  //在这个数据集的sequences中找到 配置文件中指定的sequence
                seq_idx = idx;
                break;
            }
        }
        ...
        // 通过下面的操作，使得sequences的大小为1，保存的是指定的那个sequence
...
#ifdef CT_ICP_WITH_VIZ   // 可视化操作的部分，没看懂，用了很多库和指针
    ...
    //  ***  step5: 读取数据，进行odometry操作，并对过程中的trajectory和map进行可视化
    std::map<std::string, ct_icp::seq_errors> sequence_name_to_errors;   //seq_errors指的是测试的每个sequence的 误差指标
    ...
    for (int i = 0; i < num_sequences; ++i) { //num_sequences
        ...
        // options.max_frames=-1的话，表示处理全部的帧
        while (iterator_ptr->HasNext() && (options.max_frames < 0 || frame_id < options.max_frames)) {
            ...
            // std::cout<<"=============Fame Size========"<<frame.size()<<std::endl;

            auto time_read_pointcloud = std::chrono::steady_clock::now();

            // ** 4: 对当前帧进行配准
            auto summary = ct_icp_odometry.RegisterFrame(frame);  //先初始化当前帧的motion,再
            avg_number_of_attempts += summary.number_of_attempts;
            auto time_register_frame = std::chrono::steady_clock::now();

            std::chrono::duration<double> total_elapsed = time_register_frame - time_start_frame;  // 从读取到配准完毕 需要的 时间
        
            std::chrono::duration<double> registration_elapsed = time_register_frame - time_read_pointcloud; //仅配准需要的时间
